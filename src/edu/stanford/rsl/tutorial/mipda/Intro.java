...
import static edu.stanford.rsl.conrad.numerics.SimpleOperators.multiply;
...
public class Intro {
     ...
     */
    String outputDataLoc = System.getProperty("user.home") + "/mooo/vhb/MedicalImageProcessingForDiagnosticApplications/exercises/CONRAD/output/";
	...
	public void basicIntro() {
		...
		M = new SimpleMatrix(new double[][]{{1,2,3},{4,5,6},{7,8,9}});
		if (M != null)
		...
		ResMat = multiplyMatrixProd(Mtrans, M);
		if (ResMat != null)
	    ...
	    MsquaredElem = SimpleOperators.elementWiseOperator(M, x -> x*x);
		...
		//norms
		matrixNormL1 = M.norm(MatrixNormType.MAT_NORM_FROBENIUS);
	...
	public void gridIntro(){
		...
		int centerX = imageSizeX/2;
		int centerY = imageSizeY/2;
		for (int x = 0; x<imageSizeX; x++)
			for (int y = 0; y<imageSizeY; y++) {
				int distanceFromCenter = (int) sqrt(pow(centerX - x, 2) + pow(centerY - y, 2));
				image.setAtIndex(x, y, (distanceFromCenter <= radius) ? insideVal : 0);
			}
		...
		if (mrImage != null) {
			...
			IJ.save(ImageUtil.wrapGrid(convolvedImage, "Convolved Image"), outFileName);
		}
	}
